...
pub unsafe trait Source: AsCommunicator {
    ...
    fn immediate_receive_into<'b, Buf: ?Sized>(&self, buf: &'b mut Buf) -> WriteRequest<'b, Buf>
        where Buf: 'b + BufferMut
    {
        self.immediate_receive_into_with_tag(buf, unsafe { ffi::RSMPI_ANY_TAG })
    }
...
impl Drop for Message {
    fn drop(&mut self) {
        assert!(self.as_raw() == unsafe { ffi::RSMPI_MESSAGE_NULL },
...
pub fn send_receive<R, M, D, S>(msg: &M, destination: &D, source: &S) -> (R, Status)
    where M: Equivalence,
          ...
          S: Source
{
    send_receive_with_tags(msg, destination, Tag::default(), source, unsafe { ffi::RSMPI_ANY_TAG })
}
